...
{
	/// <summary>
	/// Summary description for FaultXMLTest18.
	/// Make Faults.xml with extra <MatchParam> tag testValue
	/// </summary>
	public class FaultXMLTest18:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest18()
		{
		...
		{
			Console.WriteLine("Test Case :  Make Faults.xml with extra <MatchParam> tag testValue");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeMatchParam = xmlDocument.SelectSingleNode( "/Faults/Fault/Function/MatchParams/MatchParam" );
				
				//Element = MatchParam
				XmlElement MatchParamElement = xmlDocument.CreateElement(testValue.ToString());
				
				nodeMatchParam.AppendChild (MatchParamElement);		
