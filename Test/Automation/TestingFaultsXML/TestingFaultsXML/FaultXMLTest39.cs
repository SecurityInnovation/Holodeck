...
{
	/// <summary>
	/// Summary description for FaultXMLTest39.
	/// Make Faults.xml with 'Faults Fault Function OverrideErrorCode'missing
	/// </summary>
	public class FaultXMLTest39:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest39()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function OverrideErrorCode' missing ");
			try
			{
				string holodeckPath;
				...
				Console.WriteLine("Removing Function OverrideErrorCode Attribute from {0} of {1} fault",nodeFunction.Attributes["Name"].Value.ToString(),nodeFunction.ParentNode.Attributes["Name"].Value.ToString());

				nodeFunction.Attributes.RemoveNamedItem("OverrideErrorCode");
