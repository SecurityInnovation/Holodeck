...
{
	/// <summary>
	/// Summary description for FaultXMLTest16.
	/// Make Faults.xml with extra <MatchParams> tag testValue
	/// </summary>
	public class FaultXMLTest16:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest16()
		{
		...
		{
			Console.WriteLine("Test Case :  Make Faults.xml with extra <MatchParams> tag testValue");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeMatchParams = xmlDocument.SelectSingleNode( "/Faults/Fault/Function/MatchParams" );
				
				//Element = MatchParams
				XmlElement MatchParamsElement = xmlDocument.CreateElement(testValue.ToString());
				
				nodeMatchParams.AppendChild (MatchParamsElement);		
