...
{
	/// <summary>
	/// Summary description for FaultXMLTest25.
	/// Make Faults.xml with 'Faults Fault Function MatchParams MatchParam TestValue'missing
	/// </summary>
	public class FaultXMLTest25:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest25()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function MatchParams MatchParam TestValue' missing ");
			try
			{
				string holodeckPath;
				...
				Console.WriteLine("Removing MatchParam TestValue Attribute from {0} of {1} fault",nodeMatchParam.Attributes["Name"].Value.ToString(),nodeMatchParam.ParentNode.ParentNode.ParentNode.Attributes["Name"].Value.ToString());

				nodeMatchParam.Attributes.RemoveNamedItem("TestValue");
