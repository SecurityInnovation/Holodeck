...
{
	/// <summary>
	/// Summary description for FaultXMLTest9.
	/// Make Faults.xml with repeated Fault
	/// </summary>
	public class FaultXMLTest9:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest9()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with repeated Fault");
			try
			{
				string holodeckPath;
				...
				Console.WriteLine("Repeating {0} fault in faults.xml file ",nodeFault.Attributes["Name"].Value);
				
				XmlNode cloneNode = nodeFault.Clone();
				
				nodeFault.ParentNode.AppendChild(cloneNode);
