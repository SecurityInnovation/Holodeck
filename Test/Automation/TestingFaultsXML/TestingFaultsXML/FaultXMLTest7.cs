...
{
	/// <summary>
	/// Summary description for FaultXMLTest7.
	/// Make Faults.xml with extra <Faults> tag testValue
	/// </summary>
	public class FaultXMLTest7:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest7()
		{
		...
		{
			Console.WriteLine("Test Case :  Make Faults.xml with extra <Faults> tag testValue");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeFaults = xmlDocument.SelectSingleNode( "/Faults" );
				
				//Element = Faults
				XmlElement FaultsElement = xmlDocument.CreateElement(testValue.ToString());
				
				nodeFaults.AppendChild (FaultsElement);		
