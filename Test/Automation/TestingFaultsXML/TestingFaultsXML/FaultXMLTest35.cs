...
{
	/// <summary>
	/// Summary description for FaultXMLTest35.
	/// Make Faults.xml with 'Faults Fault Function CheckResource Exists' missing
	/// </summary>
	public class FaultXMLTest35:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest35()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function CheckResource Exists' missing");
			try
			{
				string holodeckPath;
				...
				Console.WriteLine("Removing CheckResource Exists Attribute of {0} function in {1} fault",nodeCheckResource.ParentNode.Attributes["Name"].Value.ToString(),nodeCheckResource.ParentNode.ParentNode.Attributes["Name"].Value.ToString());

				nodeCheckResource.Attributes.RemoveNamedItem("Exists");
