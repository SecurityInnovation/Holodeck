...
{
	/// <summary>
	/// Summary description for FaultXMLTest30
	/// Make Faults.xml with extra CheckResource  tag testValue
	/// </summary>
	public class FaultXMLTest30:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest30()
		{
		...
		{
			Console.WriteLine("Test Case :  Make Faults.xml with extra CheckResource tag testValue");
			try
			{
				string holodeckPath;
				...
				//Element = MatchParams
				XmlElement CheckResourceElement = xmlDocument.CreateElement(testValue.ToString());
				
				nodeCheckResource.AppendChild (CheckResourceElement);		
