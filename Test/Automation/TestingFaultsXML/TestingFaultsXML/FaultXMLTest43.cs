...
{
	/// <summary>
	/// Summary description for FaultXMLTest43.
	/// Make Faults.xml with 'Faults Fault Function PassThrough'missing
	/// </summary>
	public class FaultXMLTest43:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest43()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function PassThrough' missing ");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeFunction = xmlDocument.SelectSingleNode( "/Faults/Fault/Function[@PassThrough]" );
				Console.WriteLine("Removing Function PassThrough Attribute from {0} of {1} fault",nodeFunction.Attributes["Name"].Value.ToString(),nodeFunction.ParentNode.Attributes["Name"].Value.ToString());

				nodeFunction.Attributes.RemoveNamedItem("PassThrough");
