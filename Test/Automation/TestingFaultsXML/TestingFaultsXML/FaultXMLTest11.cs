...
{
	/// <summary>
	/// Summary description for FaultXMLTest11.
	/// Make Faults.xml with Faults Fault Name missing
	/// </summary>
	public class FaultXMLTest11:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest11()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with Faults Fault Name missing");
			try
			{
				string holodeckPath;
				...
				Console.WriteLine("Removing Name Attribute from {0} fault",nodeFault.Attributes["Name"].Value.ToString());

				nodeFault.Attributes.RemoveNamedItem("Name");
