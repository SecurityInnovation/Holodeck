...
{
	/// <summary>
	/// Summary description for FaultXMLTest27.
	/// Make Faults.xml with 'Faults Fault Function MatchParams MatchParam CompareAsType'missing
	/// </summary>
	public class FaultXMLTest27:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest27()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function MatchParams MatchParam CompareAsType' missing ");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeMatchParam = xmlDocument.SelectSingleNode( "/Faults/Fault/Function/MatchParams/MatchParam[@CompareAsType]" );
				Console.WriteLine("Removing MatchParam CompareAsType Attribute from {0} of {1} fault",nodeMatchParam.Attributes["Name"].Value.ToString(),nodeMatchParam.ParentNode.ParentNode.ParentNode.Attributes["Name"].Value.ToString());

				nodeMatchParam.Attributes.RemoveNamedItem("CompareAsType");
