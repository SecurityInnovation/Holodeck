...
{
	/// <summary>
	/// Summary description for FaultXMLTest21.
	/// Make Faults.xml with 'Faults Fault Function MatchParams MatchParam Name'missing
	/// </summary>
	public class FaultXMLTest21:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest21()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function MatchParams MatchParam Name' missing ");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeMatchParam = xmlDocument.SelectSingleNode( "/Faults/Fault/Function/MatchParams/MatchParam[@Name]" );
				Console.WriteLine("Removing MatchParam Name Attribute from {0} of {1} fault",nodeMatchParam.Attributes["Name"].Value.ToString(),nodeMatchParam.ParentNode.ParentNode.ParentNode.Attributes["Name"].Value.ToString());

				nodeMatchParam.Attributes.RemoveNamedItem("Name");
