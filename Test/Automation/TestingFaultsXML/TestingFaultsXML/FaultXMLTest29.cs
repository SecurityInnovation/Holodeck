...
{
	/// <summary>
	/// Summary description for FaultXMLTest29.
	/// Make Faults.xml with 'Faults Fault Function MatchParams MatchParam ID'missing
	/// </summary>
	public class FaultXMLTest29:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest29()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function MatchParams MatchParam ID' missing ");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeMatchParam = xmlDocument.SelectSingleNode( "/Faults/Fault/Function/MatchParams/MatchParam[@ID]" );
				Console.WriteLine("Removing MatchParam ID Attribute from {0} of {1} fault",nodeMatchParam.Attributes["Name"].Value.ToString(),nodeMatchParam.ParentNode.ParentNode.ParentNode.Attributes["Name"].Value.ToString());

				nodeMatchParam.Attributes.RemoveNamedItem("ID");
