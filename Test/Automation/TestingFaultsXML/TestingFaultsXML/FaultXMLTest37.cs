...
{
	/// <summary>
	/// Summary description for FaultXMLTest37.
	/// Make Faults.xml with 'Faults Fault Function Name'missing
	/// </summary>
	public class FaultXMLTest37:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest37()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function Name' missing ");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeFunction = xmlDocument.SelectSingleNode( "/Faults/Fault/Function[@Name]" );
				Console.WriteLine("Removing Function Name Attribute from {0} of {1} fault",nodeFunction.Attributes["Name"].Value.ToString(),nodeFunction.ParentNode.Attributes["Name"].Value.ToString());

				nodeFunction.Attributes.RemoveNamedItem("Name");
