...
{
	/// <summary>
	/// Summary description for FaultXMLTest41.
	/// Make Faults.xml with 'Faults Fault Function OverrideReturnValue'missing
	/// </summary>
	public class FaultXMLTest41:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest41()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with 'Faults Fault Function OverrideReturnValue' missing ");
			try
			{
				string holodeckPath;
				...
				XmlNode nodeFunction = xmlDocument.SelectSingleNode( "/Faults/Fault/Function[@OverrideReturnValue]" );
				Console.WriteLine("Removing Function OverrideReturnValue Attribute from {0} of {1} fault",nodeFunction.Attributes["Name"].Value.ToString(),nodeFunction.ParentNode.Attributes["Name"].Value.ToString());

				nodeFunction.Attributes.RemoveNamedItem("OverrideReturnValue");
