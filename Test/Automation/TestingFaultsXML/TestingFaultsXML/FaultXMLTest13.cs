...
{
	/// <summary>
	/// Summary description for FaultXMLTest13.
	/// Make Faults.xml with Faults Fault ReturnValue missing
	/// </summary>
	public class FaultXMLTest13:FaultXMLTest
	{
		string FaultsXMLFilePath;
		string FaultsXMLBackupFilePath;
		FileInfo modifyThisFile;

		public FaultXMLTest13()
		{
		...
		{
			Console.WriteLine("Test Case :Make Faults.xml with Faults Fault ReturnValue missing");
			try
			{
				string holodeckPath;
				...
				Console.WriteLine("Removing ReturnValue Attribute from {0} fault",nodeFault.Attributes["Name"].Value.ToString());

				nodeFault.Attributes.RemoveNamedItem("ReturnValue");
