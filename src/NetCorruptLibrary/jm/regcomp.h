...
inline bool RE_CALL reg_expression<charT, traits, Allocator>::can_start(charT c, const unsigned char* _map, unsigned char mask, const _wide_type&)
{
   if((traits_size_type)(traits_uchar_type)c >= 256)
      return true;
   return JM_MAKE_BOOL(_map[(traits_uchar_type)c] & mask);
}

template <class charT, class traits, class Allocator>
inline bool RE_CALL reg_expression<charT, traits, Allocator>::can_start(charT c, const unsigned char* _map, unsigned char mask, const _narrow_type&)
{
   return JM_MAKE_BOOL(_map[(traits_uchar_type)c] & mask);
}
...
template <class charT, class traits, class Allocator>
inline bool RE_CALL reg_expression<charT, traits, Allocator>::operator==(const reg_expression<charT, traits, Allocator>& e)const
{
   return (_flags == e.flags()) && (re_strcmp(expression(), e.expression()) == 0);
}

template <class charT, class traits, class Allocator>
bool RE_CALL reg_expression<charT, traits, Allocator>::operator<(const reg_expression<charT, traits, Allocator>& e)const
{
...
void RE_CALL reg_expression<charT, traits, Allocator>::compile_maps()
{
   ...
   while(record->type != syntax_element_match)
   {
      ...
      {
         memset(&(((re_jump*)record)->_map), 0, 256);
         record->can_be_null = 0;
         compile_map(record->next.p, ((re_jump*)record)->_map, &(record->can_be_null), mask_take, ((re_jump*)record)->alt.p);
         compile_map(((re_jump*)record)->alt.p, ((re_jump*)record)->_map, &(record->can_be_null), mask_skip);
      }
      else
      {
         record->can_be_null = 0;
   ...
   case syntax_element_jump:
      if(((re_jump*)node)->alt.p < node)
      {
         ...
         else
         {
            ...
            // skip that repeat.
            re_syntax_base* next = ((re_jump*)node)->alt.p;
            bool b = probe_start(next, cc, terminal);
            if((next->type == syntax_element_rep) && (((re_repeat*)next)->min != 0))
            {
               b = b || probe_start(((re_jump*)next)->alt.p, cc, terminal);
            }
            return b;
         }
      }
...
void RE_CALL reg_expression<charT, traits, Allocator>::compile_map(
                        re_syntax_base* node, unsigned char* _map,
...
re_syntax_base* RE_CALL reg_expression<charT, traits, Allocator>::compile_set_simple(re_syntax_base* dat, unsigned long cls, bool isnot)
{
   ...
   {
      data.align();
      dat->next.i = (unsigned int)data.size();
   }
...
re_syntax_base* RE_CALL reg_expression<charT, traits, Allocator>::compile_set(const charT*& first, const charT* last)
{
   ...
   if(has_digraphs)
      result = compile_set_aux(singles, ranges, classes, equivalents, isnot, _wide_type());
   else
...
template <class charT, class traits, class Allocator>
re_syntax_base* RE_CALL reg_expression<charT, traits, Allocator>::compile_set_aux(jstack<re_str<charT>, Allocator>& singles, jstack<re_str<charT>, Allocator>& ranges, jstack<jm_uintfast32_t, Allocator>& classes, jstack<re_str<charT>, Allocator>& equivalents, bool isnot, const _wide_type&)
{
   ...
   while(singles.empty() == false)
   {
      ...
      unsigned len = (unsigned)((re_strlen(s.c_str()) + 1) * sizeof(charT));
   ...
   while(ranges.empty() == false)
   {
      ...
      ++cranges;
      unsigned len = (unsigned)((re_strlen(c1.c_str()) + 1) * sizeof(charT));
      memcpy(data.extend(len), c1.c_str(), len);
      len = (unsigned)((re_strlen(c2.c_str()) + 1) * sizeof(charT));
...
template <class charT, class traits, class Allocator>
re_syntax_base* RE_CALL reg_expression<charT, traits, Allocator>::compile_set_aux(jstack<re_str<charT>, Allocator>& singles, jstack<re_str<charT>, Allocator>& ranges, jstack<jm_uintfast32_t, Allocator>& classes, jstack<re_str<charT>, Allocator>& equivalents, bool isnot, const _narrow_type&)
{
   ...
   while(singles.empty() == false)
   {
      dat->_map[(traits_size_type)(traits_uchar_type)*(singles.peek().c_str())] = mask_all;
      singles.pop();
   }
   while(ranges.empty() == false)
   {
   ...
   if(flags)
   {
      for(unsigned int i = 0; i < 256; ++i)
      {
         if(traits_type::is_class(charT(i), flags MAYBE_PASS_LOCALE(locale_inst)))
            dat->_map[(traits_uchar_type)traits_type::translate((charT)i, (_flags & regbase::icase) MAYBE_PASS_LOCALE(locale_inst))] = mask_all;
      }
   }
   ...
   try
   {
      ...
      while(ptr->next.i)
      {
...
#ifdef JM_DEBUG
            if((padding_mask & (int)((re_jump*)ptr)->alt.p) && (((re_jump*)ptr)->alt.p != b))
            {
               jm_trace("padding mis-aligment in repeat jump to object type: " << ((re_jump*)ptr)->alt.p->type)
               //jm_assert(0 == (padding_mask & (int)((re_jump*)ptr)->alt.p));
            }
#endif
         ...
         case syntax_element_alt:
            ...
            if((padding_mask & (int)((re_jump*)ptr)->alt.p) && (((re_jump*)ptr)->alt.p != b))
            {
               jm_trace("padding mis-aligment in alternation jump to object type: " << ((re_jump*)ptr)->alt.p->type)
               //jm_assert(0 == (padding_mask & (int)((re_jump*)ptr)->alt.p));
            }
#endif
            goto rebase;
...
#ifdef JM_DEBUG
            if((padding_mask & (int)(ptr->next.p)) && (((re_jump*)ptr)->alt.p != b))
            {
               jm_trace("padding mis-alignment in next record of type " << ptr->next.p->type)
               jm_assert(0 == (padding_mask & (int)(ptr->next.p)));
            }
#endif
            ptr = ptr->next.p;
         }
...
unsigned int RE_CALL reg_expression<charT, traits, Allocator>::set_expression(const charT* p, const charT* end, jm_uintfast32_t f)
{
   ...
   {
#ifdef JM_MSC_VER
      locale_inst = JM_STD::_ADDFAC(locale_inst, new regfacet<charT>());
#else
      locale_inst = JM_STD::locale(locale_inst, new regfacet<charT>());
#endif
   }
   ...
   while (ptr < end)
   {
      ...
      case syntax_open_bracket:
         ...
         mark.push((jm_uintfast32_t)data.index(dat));
         ++ptr;
         break;
      case syntax_close_bracket:
         ...
         {
            ((re_jump*)dat)->alt.i = (unsigned int)data.size();
            mark.pop();
      ...
      case syntax_star:
         ...
         repeat_jump:
         {
            ...
            case syntax_element_literal:
               ...
               offset = (unsigned int)((char*)dat - (char*)data.data());
               break;
            case syntax_element_backref:
            case syntax_element_long_set:
            case syntax_element_set:
            ...
            dat->next.i = (unsigned int)(((char*)dat - (char*)data.data()) + re_repeater_size);
            dat->type = syntax_element_rep;
            ((re_repeat*)dat)->alt.i = (unsigned int)data.size();
...
re_syntax_base* RE_CALL reg_expression<charT, traits, Allocator>::add_literal(re_syntax_base* dat, charT c)
{
   if(dat && (dat->type == syntax_element_literal))
   {
      ...
      JM_STDC::ptrdiff_t pos = (unsigned char*)dat - (unsigned char*)data.data();
...
unsigned int RE_CALL reg_expression<charT, traits, Allocator>::fixup_leading_rep(re_syntax_base* dat, re_syntax_base* end)
{
   ...
   while(dat != end)
   {
      ...
      case syntax_element_set:
         ...
         break;
      case syntax_element_rep:
         if((0 == len) && (1 == fixup_leading_rep(dat->next.p, ((re_repeat*)dat)->alt.p) ))
         {
