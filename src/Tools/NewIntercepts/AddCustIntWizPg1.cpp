...
namespace AddNewIntercepts
{
	...
	AddCustIntWizPg1::AddCustIntWizPg1() : WizardPage()
	{
		...
		FuncLibTreeView_AfterCheckHandler = new TreeViewEventHandler (this, FuncLibTreeView_AfterCheck);
		FuncLibTreeView->AfterCheck += FuncLibTreeView_AfterCheckHandler;
		btnBrowseOutput->Click += new EventHandler(this, btnBrowseOutput_Click);
		AddFuncLibBtn->Click += new EventHandler (this, AddFuncLibBtn_Click);
		RmFuncLibBtn->Click += new EventHandler (this, RmFuncLibBtn_Click);
		FuncLibTreeView->AfterSelect += new TreeViewEventHandler (this, FuncLibTreeView_AfterSelect);
		...
		//Add handlers
		this->lnkCleanNoDefs->LinkClicked += new System::Windows::Forms::LinkLabelLinkClickedEventHandler (this, lnkCleanNoDefs_LinkClicked);
	}
	//*************************************************************************
	// Method:		~AddCustIntWizPg1
	...
	void AddCustIntWizPg1::AddFuncLibBtn_Click(Object *sender, EventArgs *e)
	{
		...
		if (result == DialogResult::OK)
		{
			...
			waitDialog->Cancel_ClickHandler = new EventHandler (this, waitDialogCancelEventHandlerFunc);

			ThreadStart *queryThreadStart = new ThreadStart(this, GetLibraryInformationThreadFunc);
	...
	void AddCustIntWizPg1::displayNativeLibraryInformation (TreeNode * libNode)
	{
		...
		btnHeaderAdd->Click += new EventHandler (this, btnHeaderAdd_Click);
		...
		btnHeaderDel->Click += new EventHandler (this, btnHeaderDel_Click);
	...
	void AddCustIntWizPg1::displayNativeFunctionInformation (TreeNode * funcNode)
	{
		...
		rbManualEdit->CheckedChanged += new EventHandler (this, funcDef_CheckChanged);
		...
		if (funcDefinitions && funcDefinitions->Count > 0)
		{
			...
			for (int i = funcDefinitions->Count - 1; i >= 0 ; i--)
			{
				...
				rb->CheckedChanged += new EventHandler (this, funcDef_CheckChanged);
	...
	void AddCustIntWizPg1::GetLibraryInformationThreadFunc()
	{
		try
		{
			...
			// If accessing a .NET 2.0 assembly, we don't want to go further
			if (peFile->getDotNetVersion() > 1)
			{
				if (waitDialog)
					waitDialog->Close();

				MessageBox::Show (S"The library you selected appears to be a .NET version 2.0+ assembly while your system does not appear to have .NET version 2.0 or above installed on it. Please install .NET version 2.0 or above and try again.", "Holodeck", MessageBoxButtons::OK, MessageBoxIcon::Information);
				return;
			}
