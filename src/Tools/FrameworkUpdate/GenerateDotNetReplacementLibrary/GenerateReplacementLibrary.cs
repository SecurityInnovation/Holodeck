...
class GenerateReplacementLibrary
{
	...
	static void GenerateLibrary(string assemblyFileName, string iniFileName, string outFileName)
	{
		...
		System.Diagnostics.Process.Start("C:\\Program Files\\Microsoft Visual Studio .NET 2003\\SDK\\v1.1\\Bin\\ildasm.exe", assemblyFileName + " /OUT=temp.dat").WaitForExit();
		...
		foreach (Type t in typeList)
		{
			try
			{
				...
				foreach (MethodInfo m in methodList)
				{
					...
					if (!m.IsPublic)
						continue;
                    if (m.IsGenericMethod || m.ReturnType.IsGenericType)
                        continue;
                    ...
                    bool hasVoidPtrOrTypedRef = false; bool isNotHandled = false;
					for (int i=0; i<paramList.Length; i++)
					{
                        // Not handling generics
                        if (paramList[i].ParameterType.IsGenericParameter ||
                            paramList[i].ParameterType.IsGenericType ||
                            paramList[i].ParameterType.IsGenericTypeDefinition)
                        {
                            isNotHandled = true;
                            break;
                        }

                        // Not handling non-CLS types
                        if (paramList[i].ParameterType.FullName.Contains("*"))
                        {
                            isNotHandled = true;
                            break;
                        }
				...
				foreach (ConstructorInfo m in constructorList)
				{
					...
					if (hasVoidPtrOrTypedRef || isNotHandled)
						continue;
