...
namespace Crownwood.Magic.Controls
{
    ...
    public class TabControl : Panel
    {
		...
		protected bool _switchPagesOnCtrlTab;	// Switch selected tab page on Ctrl-Tab and Ctrl-Shift-Tab
			...
			if ((msg.Msg == WM_KEYDOWN) || (msg.Msg == WM_SYSKEYDOWN))
			{
				switch(keyData)
				{
					case Keys.Control | Keys.Tab:
						makeChange = true;
						newPageIndex = _pageSelected + 1;
						if (newPageIndex >= _tabPages.Count)
							newPageIndex = 0;
						break;
					...
					case Keys.Control | Keys.Tab | Keys.Shift:
						...
						newPageIndex = _pageSelected - 1;
						if (newPageIndex < 0)
							newPageIndex = _tabPages.Count - 1;
						break;
				}
			}

			if (makeChange && SwitchTabPagesOnCtrlTab)
			{
				MovePageSelection(_tabPages[newPageIndex]);
				MakePageVisible(_tabPages[newPageIndex]);

				// Set focus into the selected page contents
				if (_tabPages[newPageIndex].Control != null)
					_tabPages[newPageIndex].Control.Focus();
				else
					_tabPages[newPageIndex].Focus();
		...
		public void SimulateCtrlTabPress()
		{
			int newPageIndex = _pageSelected + 1;
			if (newPageIndex >= _tabPages.Count)
				newPageIndex = 0;
		...
		public void SimulateCtrlShiftTabPress()
		{
			int newPageIndex = _pageSelected - 1;
			if (newPageIndex < 0)
				newPageIndex = _tabPages.Count - 1;
		...
		public bool SwitchTabPagesOnCtrlTab
		{
			get { return this._switchPagesOnCtrlTab; }
			set { this._switchPagesOnCtrlTab = value; }
		}
        ...
        protected virtual void HandleShowingTabPage(TabPage page, Control c)
        {
            ...
            if (!page.Shown)
            {
                ...
                // TODO: Verify if this code is correct; changed AutoScale to AutoScaleMode
                    ...
                    if (_tabsAreaRect.Contains(mousePos))
                    {
                        ...
                        if (!ce.Cancel)
                        {
                            ...
                            if (_contextMenu != null)
                                _contextMenu.TrackPopup(this.PointToScreen(new Point(e.X, e.Y)), false);
                        }
                    }
                }
            }
