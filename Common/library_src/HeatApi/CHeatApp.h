...
class CHeatApp
{
	...
	DWORD delayedInjectModule(LPSTR pszDLLName);
	DWORD ejectModule(LPSTR pszDLLName);
	DWORD sendIntercepts();
	DWORD sendUnintercept(LPSTR pszFuncName, LPSTR pszFuncDLL);
	void  startMonitorThread();

	friend DWORD WINAPI delayedInjectThread(LPVOID pCHeatApp);
...
public:
	...
	DWORD delayedInject();									// Injects the HEAT DLL into an uninitialized process
	...
	DWORD interceptFunc(PVOID pFuncToIntercept, LPSTR pszHandlerFunc, LPSTR pszHandlerDLL);

	DWORD unInterceptFunc(LPSTR pszFuncName, LPSTR pszFuncDLL);
