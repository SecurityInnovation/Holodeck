...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (!IsCommandLine)
	{
		...
		if (!CLRAddInterceptHandlerModuleForCacheCheck("MSCorLibReplacementLibrary.dll"))
		{
			MessageBox(NULL, "Could not access .NET replacement libraries.  Please reinstall Holodeck.", "Error", MB_OK | MB_ICONSTOP | MB_TOPMOST | MB_TASKMODAL);
			if (fileToWriteOnFailure != NULL)
				File::OpenWrite(fileToWriteOnFailure)->Close();
		...
		if (!CLRAddInterceptHandlerModuleForCacheCheck("SystemReplacementLibrary.dll"))
		{
			MessageBox(NULL, "Could not access .NET replacement libraries.  Please reinstall Holodeck.", "Error", MB_OK | MB_ICONSTOP | MB_TOPMOST | MB_TASKMODAL);
			if (fileToWriteOnFailure != NULL)
				File::OpenWrite(fileToWriteOnFailure)->Close();
		...
		if (!CLRAddInterceptHandlerModuleForCacheCheck("SystemServiceProcessReplacementLibrary.dll"))
		{
			MessageBox(NULL, "Could not access .NET replacement libraries.  Please reinstall Holodeck.", "Error", MB_OK | MB_ICONSTOP | MB_TOPMOST | MB_TASKMODAL);
			if (fileToWriteOnFailure != NULL)
				File::OpenWrite(fileToWriteOnFailure)->Close();
		...
		if (!CLRAddInterceptHandlerModuleForCacheCheck("SystemXMLReplacementLibrary.dll"))
		{
			MessageBox(NULL, "Could not access .NET replacement libraries.  Please reinstall Holodeck.", "Error", MB_OK | MB_ICONSTOP | MB_TOPMOST | MB_TASKMODAL);
			if (fileToWriteOnFailure != NULL)
				File::OpenWrite(fileToWriteOnFailure)->Close();
