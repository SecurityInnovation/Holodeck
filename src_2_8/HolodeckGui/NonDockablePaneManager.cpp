...
namespace HolodeckGui
{
	...
	void NonDockablePaneManager::onSelectionChanged(Object *sender, EventArgs *args)
	{
		...
		if (selectedPage)
		{
			...
			{
				NetworkLogPane *logPane = dynamic_cast<NetworkLogPane *>(selectedPage->Control);
				dataNode = new DisplayableDataNode("Network Logs", UIData::ActiveNetworkLogPane, logPane->ProcessID, logPane->ThreadID);
				dataNode->DataArray = logPane->DataArray;
			}
			...
			else if (type->Name->CompareTo("NetworkCorruptionPane") == 0)
			{
				...
				dataNode = new DisplayableDataNode("Network Fuzzing Fault", UIData::NetworkCorruptionFault, networkCorruptionPane->ProcessID, 0);
			...
			else if (type->Name->CompareTo("ResourceTestPane") == 0)
			{
				...
				dataNode = new DisplayableDataNode("Resource Fault", UIData::ResourceFault, resourceTestPane->ProcessID, 0);
				dataNode->DataArray = resourceTestPane->DataArray;
			}
			else if (type->Name->CompareTo("HelpContentsPane") == 0)
			{
				dataNode = new DisplayableDataNode("", UIData::NodeType::HelpDetails, 0, 0);
			}
			
			if (updateOnFocusChange && (dataNode != NULL))
	...
	void NonDockablePaneManager::displayHelpPane(String *location)
	{
		...
		if ((File::Exists(location)) || (location->ToLower()->StartsWith("file:")) || (location->ToLower()->StartsWith("about:")))
		{
			updateOnFocusChange = false;
